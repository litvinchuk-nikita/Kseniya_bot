...
# Этот хэндлер будет срабатывать на команду /end_draw
# и переводить бота в состояние проведения розыгрыша
@router.message(Command(commands='end_draw'), StateFilter(default_state))
async def process_choose_command(message: Message, state: FSMContext):
