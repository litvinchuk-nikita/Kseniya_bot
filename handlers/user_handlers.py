...
#     await message.answer(
#         text=f'Вы находитесь в состоянии бронирования мероприятия\n'
...
# # Этот хэндлер будет срабатывать на callback showreservation
...
# async def process_showreservation_command(callback: CallbackQuery):
...
#             num += 1
#         bookings = f'\n\n'.join(booking_list)
#         await callback.message.answer(text=f"Забронированные мероприятия:\n{bookings}")
#     else:
...
# # Этот хэндлер будет срабатывать на callback cancelreservation
# # и отправлять в чат данные о бронировании и вопрос о снятии бронирования
# @router.callback_query(Text(text='cancelreservation'), StateFilter(default_state))
# async def process_cancelreservation_command(callback: CallbackQuery, state: FSMContext):
#     # Отправляем пользователю информацию о бронировании, если оно есть в базе данных
#     reserv_list = select_reserv_db(str(callback.from_user.id))
#     if len(reserv_list) != 0:
#         booking_list = []
...
#                              'Если вы хотите прервать процесс отмены бронирования - '
#                              'введите команду - /cancel')
...
# @router.message(StateFilter(FSMCancelReserv.cancel_reservation))
# async def del_reserv(message: Message):
#     await message.answer(
#         text=f'Вы находитесь в состоянии отмены бронирования\n'
#              f'Для отмены бронирования введите номер бронирования\n'
#              'Если вы хотите прервать процесс отмены бронирования - '
#              'введите команду - /cancel')
...
#     await message.answer(
#         text=f'Вы находитесь в состоянии удаления мероприятия\n'
...
async def del_event(message: Message):
    await message.answer(
        text=f'Вы находитесь в состоянии отмены бронирования\n'
             f'Для отмены бронирования введите номер бронирования\n'
             'Если вы хотите прервать отмену бронирования - '
             'отправьте команду /cancel')
