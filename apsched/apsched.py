...
async def send_message_cron(bot: Bot):
    ...
    for reserv in reserv_list:
        if reserv['date'] == now_date():
            try:
                text = f'Привет, дорогой зритель!\nВы забронировали зрительские места на "{reserv["event"]}" {reserv["date"]}\nКоличество забронированных мест - {reserv["guests"]}\nМесто проведения: {reserv["place"]}\nСбор гостей начинается в {reserv["entry"]}, начало мероприятия в {reserv["start"]}\n\n<i>В случае, если вы не придёте бронирование будет аннулировано в момент начала мероприятия, а ваши места предоставлены другим зрителям.</i> ⚠️\n\n‼️ если по какой-то причине вы не можете посетить наше мероприятие, пожалуйста, отмените бронь с помощью команды ниже(а если вы твердо уверены, что придёте – вы просто супер)\n\n/cancelreservation'
                photo = reserv['photo']
                if photo is None:
